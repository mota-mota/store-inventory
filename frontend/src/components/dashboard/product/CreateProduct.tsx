import {
    ...
    Input,
    Textarea,
    Form, NumberInput, addToast, Select, SelectItem,
} from "@heroui/react";
...
export function CreateProduct({isOpen, onOpenChange, product, callback}: CreateProductProps) {
    ...
    const {loading: createLoading, error: createError, response: createResponse, createProduct} = useCreateProduct();
    ...
    const {loading: updateLoading, error: updateError, response: updateResponse, updateProduct} = useUpdateProduct();
        ...
        <>
            <Modal isOpen={isOpen} placement="top-center" onOpenChange={onOpenChange}>
                <ModalContent>
                    {(onClose) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1">{product?.id ? "Editar" : "Crear"} Producto</ModalHeader>
                            <ModalBody>
                                <Form
                                    ref={formRef}
                            ...
                            <ModalFooter>
                                <Button color="danger" variant="flat" onPress={onClose} type="button" disabled={createLoading}>
                                    Cancelar
                                </Button>
                                <Button 
                                    color="primary"
